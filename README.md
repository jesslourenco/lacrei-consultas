...
Ambiente: `Docker` com `Docker-Compose`. A aplicação foi completamente containerizada, principalmente para garantir que o código seja executado da mesma forma em qualquer máquina. O Docker Compose foi utilizado para facilitar sua configuração e modo de rodar o projeto.

Gerenciador de pacotes: `Poetry`. Usado no projeto para simplificar o gerenciamento e instalação de dependências no container.
...
Dependências:
- `bleach:` biblioteca usada para sanitização de HTML, garantindo a remoção de inputs potencialmente maliciosos, como scripts XSS. Utilizada no projeto para limpar inputs de texto que serão armazenados no banco de dados, garantindo segurança contra ataques de injeção de HTML/JavaScript.
...
## Roadmap e Melhorias
- [ ] Adicionar funcionalidade de autenticação e autorização de usuários.
- [ ] Implementar notificações assíncronas por e-mail ou SMS.
- [ ] Realizar validação de CEP e endereço via chamada a uma API externa (ex.: ViaCEP).
- [ ] Adicionar um dashboard com estatísticas, como o total de consultas realizadas por profissional.
- [ ] Implementar funcionalidade de avaliação de profissionais por pacientes.
- [ ] Adicionar logs de auditoria e observabilidade, registrando eventos importantes no sistema.
- [ ] Implementar uma busca "inteligente" por profissionais, integrando com a Google Maps API para sugerir profissionais próximos ao paciente.
